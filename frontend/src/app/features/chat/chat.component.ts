    ...
    <div class="chat-layout">
      ...
      <aside class="chat-sidebar" aria-label="Select document to chat with">
        <div class="sidebar-head">
          <h3 class="sidebar-label">Documents</h3>
        </div>
          ...
          @for (doc of (docState.documents$ | async) ?? []; track doc.id) {
            @if (doc.processingStatus === 'ready') {
              <button
                ...
                (click)="selectDoc(doc)"
                role="listitem"
                [attr.aria-label]="'Chat with ' + doc.name"
      ...
      <div class="chat-main">
        @if (!activeDoc()) {
          ...
          <div class="chat-header">
            ...
            <button class="btn-clear" (click)="clearChat()" aria-label="Clear chat history">
              Clear
            </button>
          </div>
              ...
              <div class="chat-hints">
                ...
                <div class="hint-chips">
                  @for (hint of getSuggestedQuestions(); track hint) {
                    <button class="hint-chip" (click)="useHint(hint)" [attr.aria-label]="'Ask: ' + hint">
                      {{ hint }}
                    </button>
                  }
                </div>
              ...
              <div class="message" [class]="'msg-' + msg.role" [attr.aria-label]="msg.role + ' says: ' + msg.content">
                <div class="msg-avatar" aria-hidden="true">
                  @if (msg.role === 'user') {
                    ...
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                      <rect x="1" y="1" width="12" height="12" rx="3" stroke="currentColor" stroke-width="1.3" fill="none"/>
                ...
                <div class="msg-body">
                  ...
                  <time class="msg-time" [dateTime]="msg.timestamp | date:'yyyy-MM-ddTHH:mm'">
            ...
            <form class="input-form" (ngSubmit)="sendMessage()" aria-label="Chat input form">
              <div class="input-wrap">
                ...
                <button
                  type="submit"
                  class="send-btn"
                  [disabled]="!inputControl.valid || isLoading()"
...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  sendMessage(): void {
    ...
    this.api.chat(this.activeDoc()!.id, this.messages()).pipe(
      ...
      catchError(err => {
        this.error.set(err?.error?.error ?? 'Failed to get a response. Please try again.');
  ...
  useHint(hint: string): void {
    this.inputControl.setValue(hint);
    this.sendMessage();
  }
  ...
  getSuggestedQuestions(): string[] {
    ...
    const q = [
      ...
      `What is the date in this document?`,
